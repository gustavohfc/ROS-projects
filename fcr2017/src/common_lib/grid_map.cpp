...
void GridMap::updateGrid()
{
    ...
    for (double angle = laser_sensor.getAngleMin(), max = laser_sensor.getAngleMax(); angle < max; angle += GRID_MAP_LASER_ANGLE_STEP)
    {
        ...
        for (double distance = GRID_MAP_DISTANCE_STEP;
            distance < objectDistance && distance < GRID_MAP_LASER_MAX_DISTANCE && is_inside_grid;
            distance += GRID_MAP_DISTANCE_STEP)
        {
            ...
            is_inside_grid = markCoordinateAs(absolute_x, absolute_y, EMPTY_CELL);
        }
